...
Function GetRequiredPermissions([string] $applicationDisplayName, [string] $requiredDelegatedPermissions, [string]$requiredApplicationPermissions, $servicePrincipal)
{
    ...
    {
        $sp = Get-AzureADServicePrincipal -All $true -Filter "DisplayName eq '$applicationDisplayName'"
    }
    $appid = $sp.AppId
...
#> 
   ...
   Write-Host "Creating the AAD application (PowerAutomate-Flow-WebApp)"
   ...
   $serviceAadApplication = New-AzureADApplication -DisplayName "PowerAutomate-Flow-WebApp" `
                                                   -ReplyUrls "http://localhost:30662/index.html" `
   ...
   { 
        ...
        Write-Host "[Optional]'$($user.UserPrincipalName)' added as an application owner to app '$($serviceServicePrincipal.DisplayName)'"
   }
   ...
   Write-Host "Done creating the service application (PowerAutomate-Flow-WebApp)"
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>PowerAutomate-Flow-WebApp</a></td><td>$serviceAppKey</td></tr>" -Path createdApps.html
   ...
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "Microsoft Flow Service" -requiredDelegatedPermissions "User|Flows.Read.All|Flows.Manage.All|Activity.Read.All|Approvals.Manage.All" 
                                                #-requiredDelegatedPermissions "Flows.Read.All"
   $requiredResourcesAccess.Add($requiredPermissions)

   <#
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "Flow Service" `
                                                -requiredDelegatedPermissions "Activity.Read.All" `
   $requiredResourcesAccess.Add($requiredPermissions)

   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "Flow Service" `
                                                -requiredDelegatedPermissions "Approvals.Manage.All" `
   $requiredResourcesAccess.Add($requiredPermissions)

   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "Flow Service" `
                                                -requiredDelegatedPermissions "Flows.Manage.All" `
   $requiredResourcesAccess.Add($requiredPermissions)
